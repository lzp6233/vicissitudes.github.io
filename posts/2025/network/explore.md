...
> 以下是关于 **TCP Zero Window (`RcvWindow = 0`)** 处理机制的简洁整理：
>
> ### 1. 核心现象与目的
>
> - **现象:** Receiver 通知 `rwnd=0`。
> - **直接动作:** Sender **立即停止**发送新的应用层数据。
> - **目的:** **Flow Control**（流量控制），防止接收方缓存溢出导致丢包。
>
> ### 2. 潜在风险：死锁 (Deadlock)
>
> - **场景:** Receiver 腾出空间后发送的 **Window Update**（窗口更新）报文在网络中**丢失**。
> - **后果:** Sender 以为窗口仍为 0（傻等）；Receiver 以为 Sender 已收到通知（傻等数据）。双方陷入无限等待。
>
> ### 3. 解决方案：坚持定时器 (Persist Timer)
>
> 为了打破死锁，Sender 采取主动措施：
>
> 1. **启动计时:** 收到零窗口通知时，启动 **Persist Timer**。
> 2. **发送探测:** 计时器超时后，Sender 发送 **Zero Window Probe**（零窗口探测报文，通常包含 1 字节数据）。
> 3. **强制响应:** TCP 规定 Receiver **必须**确认探测报文，从而在 ACK 中带回最新的 `rwnd` 值。
>    - 若仍为 0：Sender 重置计时器继续等待。
>    - 若 > 0：Sender 恢复数据传输。
>
> ### 4. 性能优化：糊涂窗口综合症 (Silly Window Syndrome)
>
> 为了防止“刚空出 1 字节就通知发送，导致网络充满小包”的低效行为：
>
> - **接收方策略 (Clark's Algorithm):** 即使有少量空间，也暂时“撒谎”通告 `rwnd=0`。
> - **通告时机:** 直到缓存空间足以容纳 **1 个 MSS** 或者 **空出一半缓存**时，才发送窗口更新通知。
